...
class Signature:
    ...
    def __init__(self, id, up_genes, down_genes):
        self.id = id
        self.up_genes = np.array(up_genes)
        self.down_genes = np.array(down_genes)
...
class Signature_pair:
    ...
    def get_up(self):
        ...
        return np.array(up_down[0])

    def get_down(self):
        up_down = self.get_up_down()
        return np.array(up_down[1])
...
class Gene_vector:
    ...
    def coordinates(self):
        if self.gene_space.shape[0] !=0:
            coordinate = np.array(list(map(lambda x: 1 if x in self.gene_list else 0, self.gene_space)))
        else:
            coordinate = np.array([])
        return coordinate
...
@jit
def create_inf_score_as_weights_vector(array_gene_inf_score, space):
    """
    ...
    """
    #print('пространство', type(space), space.shape, space)
    #print('пространство', type(space), space[0:10])
    #print('массив словарь', type(array_gene_inf_score), array_gene_inf_score[0:10,0:2])
    #print('срез массива-словаря', array_gene_inf_score[:,0])
    inf_score_as_weights_vector = np.ones(len(space))
    n = len(space)
    for i in range(n):
    ...
    if space.shape[0] != 0:
        ...
        inf_score_as_weights_vector = np.array([])
    return inf_score_as_weights_vector
    """
...
def find_cosine_dist(pair, query_signature, array_gene_inf_score):
    """
    ...
    space_1 = np.array(list(set(query_signature.down_genes) | set(pair.get_up())))
    ...
    vector_inf_score_space_1 = create_inf_score_as_weights_vector(array_gene_inf_score, space_1)
    cosine_distance_1 = cosine(pair_up_vector.coordinates(), query_down_vector.coordinates(), vector_inf_score_space_1)


    space_2 = np.array(list(set(query_signature.up_genes) | set(pair.get_down())))
    ...
    vector_inf_score_space_2 = create_inf_score_as_weights_vector(array_gene_inf_score, space_2)
...
def cosine_dist_for_multiprocessing(i, j, query_signature, array_gene_inf_score, signature_list):
    start_time = time.time()
    pair = Signature_pair(signature_list[i], signature_list[j])
    #print('время создания одной пары:', '--- %s seconds ---' % (time.time() - start_time))
    start_time = time.time()
    print('косинусное расстояние :', find_cosine_dist(pair, query_signature, array_gene_inf_score))
    print('время работы поиска косинусного расстояния для одной пары:', '--- %s seconds ---' % (time.time() - start_time))
    return (i, j, find_cosine_dist(pair, query_signature, array_gene_inf_score))
    #return matrix
...
def cosine_similarity(content_of_file_with_signatures, df_inf_score, number_processes):
    ...
    """
    print(df_inf_score)
    list_inf_score = np.array(list(df_inf_score['inf_score']))
    list_gene = np.array(list(df_inf_score.index))
    ind = np.lexsort((list_inf_score, list_gene))
    array_gene_inf_score = np.array([(list_gene[i], list_inf_score[i]) for i in ind])
    print(array_gene_inf_score[0:10, 0:2])
    ...
    results = pool.starmap(cosine_dist_for_multiprocessing, [(i, j, query_signature, array_gene_inf_score, signature_list) for i in range(len(signature_list)) for j in range(len(signature_list)) if i < j])
